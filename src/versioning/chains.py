"""Version-chain grouping and deduplication for retrieved chunks."""
...
def chain_key(chunk: Chunk) -> tuple[str, str]:
    """Return the version-chain identifier for a chunk.

    A version chain is the set of all documents for the same company and
    document type ordered by ``doc_version``.  Two chunks share a chain if
    and only if they have the same ``(company, doc_type)`` pair.

    Args:
        chunk: The chunk whose chain key is needed.

    Returns:
        ``(company, doc_type)`` as a 2-tuple of strings.
    """
    return (chunk.company, chunk.doc_type)
...
def dedupe_by_chain(
    ...
    * ``"latest"``     — for every version chain, keep ONLY chunks whose
                         ``doc_version`` equals the maximum ``doc_version``
                         present in that chain among the retrieved set.
                         All other (older-version) chunks are dropped.
    * ``"comparison"`` — keep every chunk unchanged; the caller wants to see
                         multiple versions explicitly.
    * ``"historical"`` — keep every chunk unchanged; retrieval ranking is
                         already biased to the period named in the query.

    Chains with only one ``doc_version`` represented (e.g. VICI, EGP, RLT)
    are a no-op under all three intents: there is nothing to suppress.
    ...
    # Find the maximum (most-recent) doc_version per chain among the
    # retrieved set.  doc_version is "YYYY-MM", so lexicographic comparison
    # is equivalent to chronological comparison.
    latest_version: dict[tuple[str, str], str] = {}
    for rc in retrieved:
        key = chain_key(rc.chunk)
    ...
    return [
        rc
        for rc in retrieved
        if rc.chunk.doc_version == latest_version[chain_key(rc.chunk)]
    ]
...
def expand_to_parents(
    ...
    needs_expansion: dict[UUID, RetrievedChunk] = {}  # parent_id -> winner rc
    parent_order: list[UUID] = []   # order of first appearance of parent_id
    passthrough: list[tuple[int, RetrievedChunk]] = []  # (position, rc)
    ...
    # Build output: parent-expanded slots in first-appearance order,
    # interleaved with passthrough slots by original position.
    #
    # Strategy: assign each output item its original "slot" position,
    # then sort by that position.
